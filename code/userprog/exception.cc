...
void Copy_Mem(int registro, char* buffer) {   //Pasa lo que hay en el registro a un String
    int carac = 1;
    int i;
...
void Nachos_Read() {    //System call # 6
    ...
    case ConsoleOutput:
        ...
        break;
    case ConsoleInput:
        cant = read(1, bufferRead, tama);   //lee con el read de UNIX
    ...
    default:
        ...
        if(currentThread->tablaFiles->isOpened(idFileNachOS)) { //esta abierto?
            ...
            cant = read(idUnix, bufferRead, tama);    //lee con el read de UNIX
            machine->WriteRegister(2, cant);
            stats->numDiskReads++;
        }
        else {
