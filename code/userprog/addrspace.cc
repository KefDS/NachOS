...
AddrSpace::AddrSpace (OpenFile* executable) {
	...
	//llenando el vector que dice cuantas paginas de cada segmento (se necesecita en Memoria Virtual)
    ...
    //ASSERT (numPages <= NumPhysPages);	ya no importa xq ya tenemos virtual memory
...
void AddrSpace::load (int missingAddr) {
	...
	if (pageTable[missingPage].valid == false) { // La página faltante no está en memoria
		...
		// La entrada a reemplazar está ocupada
		if (machine->tlb[posReemplazo].valid) {
			//Eliminar una entrada del TLB (deben actualizarse las banderas entre tlb & pageTable)
